...
assert.deepEqual(targets, ['apex://MatchSportsProgramsAction', ...Array(3).fill('standardInvocableAction://streamKnowledgeSearch'), 'apex://ReviewSportsSupportReplyAction', 'apex://PrepareSportsSupportAction', 'apex://CreateSportsSupportCaseAction']);
...
assert.match(source, /available when @variables\.support_token and @variables\.support_reply == "CONFIRM"/);
assert.match(source, /available when not @variables\.support_token[^\n]+@variables\.support_reply != "CONFIRM"/);
