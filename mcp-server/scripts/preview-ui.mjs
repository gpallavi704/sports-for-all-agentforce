...
const frame=document.querySelector('iframe');const bridge=new AppBridge(null,{name:'Local UI test',version:'1'},{openLinks:{},downloadFile:{}},{hostContext:{theme:'light',displayMode:'inline'}});
...
bridge.ondownloadfile=async({contents})=>{const r=contents[0]?.resource;if(contents.length!==1||r?.uri!=='file:///sport-compass-first-visit.html'||r?.mimeType!=='text/html'||typeof r.text!=='string'||r.text.length>20000)return {isError:true};const url=URL.createObjectURL(new Blob([r.text],{type:r.mimeType}));const a=document.createElement('a');a.href=url;a.download='sport-compass-first-visit.html';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);document.getElementById('event').textContent='Visit plan download requested from local fixture.';return {};};
...
const exportPreview=buildVisitExport(fixture.clubs[0],['Do you currently offer introductory wheelchair fencing and coaching for someone new to the sport?','Is there step-free entry and an accessible restroom, and are there access arrangements I should discuss before visiting?','Do you have loaner fencing gear and a suitable fencing frame and chair setup, and what should I bring?']);
const server=createServer((req,res)=>{const routes={'/':[html,'text/html'],'/host.js':[js.outputFiles[0].text,'text/javascript'],'/widget':[widget,'text/html'],'/export-preview':[exportPreview.text,'text/html']};const item=routes[req.url];if(!item){res.writeHead(404).end();return;}res.setHeader('Content-Type',item[1]);res.setHeader('Cache-Control','no-store');res.end(item[0]);});
